...
def generate_jev_triage(report_id: int, staff: User = Depends(require_roles("admin", "case_handler")), db: Session = Depends(get_db)):
    ...
    score, level, reasons = calculate_risk(triage.ai_analysis)
    ...
    row.abuse_types = triage.ai_analysis.abuse_types
    row.threat_indicators = {
        "threat_detected": triage.ai_analysis.threat_detected,
        "weapon_mentioned": triage.ai_analysis.weapon_mentioned,
        "recurring_incident": triage.ai_analysis.recurring_incident,
        "stalking_detected": triage.ai_analysis.stalking_detected,
        "immediate_danger": triage.ai_analysis.immediate_danger,
        "key_indicators": triage.ai_analysis.key_indicators,
        "reasons": reasons,
        "source": "jev",
        "jev": _jev_payload(triage),
    }
    row.ai_summary = triage.ai_analysis.summary
